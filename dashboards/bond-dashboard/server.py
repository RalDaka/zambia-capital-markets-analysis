...
BOND_DIR = os.path.join(BASE_DIR, "bond-dashboard")
KEY_PATH = os.path.join(BASE_DIR, "kakuleta-key.json")
BOZ_BONDS_URL = "https://www.boz.zm/markets-securities/bonds"
INVITATION_COLLECTION = "grzSecuritiesInvitations"
INVITATION_DOCUMENT = "bondInvitations"
YIELD_COLLECTION = "boz_bond_yield_rates"
YIELD_DOCUMENT = "yieldUpdates"

STAGE_FILES = [
    "Monthly_Stage1_Boz_Bond_Invitations.py",
    "Monthly_Stage1a_Boz_Bond_Notifications.py",
    "Monthly_Stage2_Boz_Bond_yield_rates.py",
    "Monthly_Stage2a_Boz_Bond_Yield_Notifications.py",
    "Monthly_Stage3_Boz_Bond_add_new_yieldRates_to_Historic_Yields.py",
    "Monthly_Stage4_Boz_Bond_AmountBidAllocated.py",
    "Monthly_Stage5_Boz_Bond_Analysis.py",
]
...
def build_overview():
    found = [
        ...
        if os.path.exists(os.path.join(BOND_DIR, name))
    ]
...
def validate_yield_payload(payload):
    ...
    if not isinstance(bonds, list) or len(bonds) == 0:
        raise ValueError("At least one bond entry is required.")

    validated_bonds = []
    for bond in bonds:
        period = str(bond.get("period", "")).strip()
        isin = str(bond.get("isin", "")).strip()
        try:
            yield_val = float(bond.get("yield", 0))
            coupon_val = float(bond.get("coupon", 0))
            change_val = float(bond.get("change", 0))
        except (TypeError, ValueError):
            raise ValueError(f"Invalid numeric value in bond entry: {bond}")

        if not period:
            raise ValueError("Bond period is required.")
...
def ingest_yields(payload):
    ...
    # Remove SERVER_TIMESTAMP before returning (Sentinel is not JSON serializable)
...
def build_driver():
    ...
    if os.environ.get("BOND_HEADLESS", "1") != "0":
...
def extract_pdf_data(pdf_url):
    ...
    tender_match = re.search(r"Auction\s+Number\s+([\d/A-Z]+)", text, re.IGNORECASE)
    if tender_match:
    ...
    settlement_match = re.search(r"Settlement\s+will\s+be\s+on\s+(.*?)\.", text, re.IGNORECASE)
    if settlement_match:
...
def scrape_invitation_draft():
    ...
    try:
        driver.get(BOZ_BONDS_URL)
        wait = WebDriverWait(driver, 25)

        invitation_button = wait.until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(text(), 'View Bond Invitations')]"))
        )
        ...
        for row in rows:
            try:
                ...
                pdf_url = urljoin(BOZ_BONDS_URL, href)
        ...
        if not latest:
            return {
                ...
                "draft": None,
                "decision": "No bond invitation documents were found on the BoZ page.",
            }
...
def scrape_yield_draft():
    ...
    Duplicates only the scraping logic from Monthly_Stage2_Boz_Bond_yield_rates.py."""
    from selenium.webdriver.common.by import By
    from selenium.webdriver.support import expected_conditions as EC
    from selenium.webdriver.support.ui import WebDriverWait
    ...
    try:
        ...
        wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "app-bond-tender-result table")))

        rows = driver.find_elements(By.CSS_SELECTOR, "app-bond-tender-result tbody tr")
        bond_data = []

        for row in rows:
            cols = row.find_elements(By.TAG_NAME, "td")
            if len(cols) >= 4:
                period_raw = cols[0].text.strip()
                period_key = f"{period_raw} Years"
                isin = cols[1].text.strip()

                try:
                    yield_val = float(cols[3].text.replace(",", "").strip())
                    coupon_val = float(cols[2].text.replace(",", "").strip())

                    # Calculate change from Firestore data if available
                    previous_yield = 0.0
                    if current and "bonds" in current:
                        for bond in current["bonds"]:
                            if bond.get("period") == period_key:
                                previous_yield = bond.get("yield", 0.0)
                                break
...
def run_script(script_name, confirm=False):
    ...
    script_path = os.path.join(BOND_DIR, script_name)
...
class BondDashboardHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == "/api/bond/invitations/latest":
                self.respond_json({"invitation": get_current_invitation()})
                return

            if path == "/api/bond/invitations/scrape-draft":
            ...
            if path == "/api/bond/yields/scrape-draft":
            ...
            if path == "/api/bond/invitations/firebase":
                add_log("Loading current invitation data from Firebase...")
                stage_state["activeStage"] = "Stage 1"
                try:
                    data = get_current_invitation()
                    self.respond_json({
                        "source": "firebase",
                        "draft": data if data else None,
                        "decision": "Loaded from Firebase." if data else "No invitation data found in Firebase.",
                    })
                except Exception as e:
                    self._handle_api_error(str(e))
                return

            if path == "/api/bond/yields/firebase":
                ...
                try:
                    ...
                    self.respond_json({
                        "source": "firebase",
                        "draft": data if data else None,
                        "decision": "Loaded from Firebase." if data else "No yield data found in Firebase.",
                    })
                except Exception as e:
                    self._handle_api_error(str(e))
                return

            if path == "/api/bond/yields/latest":
    ...
    def do_POST(self):
        ...
        try:
            if path == "/api/bond/invitations/ingest":
                try:
            ...
            if path == "/api/bond/yields/ingest":
                try:
            ...
            if path == "/api/stage/run":
                ...
                script_map = {
                    "stage1a": "Monthly_Stage1a_Boz_Bond_Notifications.py",
                    "stage2": "Monthly_Stage2_Boz_Bond_yield_rates.py",
                    "stage2a": "Monthly_Stage2a_Boz_Bond_Yield_Notifications.py",
                    "stage3": "Monthly_Stage3_Boz_Bond_add_new_yieldRates_to_Historic_Yields.py",
                    "stage4": "Monthly_Stage4_Boz_Bond_AmountBidAllocated.py",
                    "stage5": "Monthly_Stage5_Boz_Bond_Analysis.py",
                }
...
def main():
    ...
    server = ThreadingHTTPServer(("127.0.0.1", port), BondDashboardHandler)
    print(f"Bond dashboard running at http://127.0.0.1:{port}")
