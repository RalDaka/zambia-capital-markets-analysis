...
# DATA LOADING
# ──────────────────────────────────────────────
BASE = "zambia-capital-markets-analysis/data/processed"


@st.cache_data
def load_prices():
    df = pd.read_csv(f"{BASE}/luse_historical_prices_clean.csv", parse_dates=["date"])
...
@st.cache_data
def load_index():
    df = pd.read_csv(f"{BASE}/luse_index.csv", parse_dates=["index_date"])
